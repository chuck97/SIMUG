...
double ComputeRelativeL2Error(IceMesh* ice_mesh, INMOST::Tag mass_tag, auto scalar_func)
{
    ...
    INMOST::Tag geo_coords_tag = ice_mesh->GetGridInfo(mesh::gridElemType::Trian)->coords[coord::coordType::geo];
    ...
    for(auto trianit = mesh->BeginCell(); trianit != mesh->EndCell(); ++trianit)
    {
        ...
        {
            double lon = trianit->RealArray(geo_coords_tag)[0];
            double lat = trianit->RealArray(geo_coords_tag)[1];
            double trian_area = trianit->Real(trian_area_tag);
            double exact_value = scalar_func(std::pair<double, double>{lon, lat}, 0.0)[0];
            double computed_value = trianit->Real(mass_tag);
            local_sum_sqr_diff += (computed_value - exact_value)*(computed_value - exact_value)*trian_area;
            local_sum_sqr_init += exact_value*exact_value*trian_area;
        }
    }
    BARRIER
   ...
   MPI_Allreduce(&local_sum_sqr_init, &all_sum_sqr_init, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    ...
    double L2_error = std::sqrt(all_sum_sqr_diff/all_sum_sqr_init);
               ...
               const std::string& output_folder)             // output folder
{
    ...
    // mesh initialization
    IceMesh* mesh_sphere = new IceMesh(mesh_path, output_folder, mesh::surfType::sphere, mesh::gridType::Cgrid); 
