...
{
    if (name == (std::string)"TG2")
    {
        return SIMUG::adv::timeScheme::TG2;
    }
    else if (name == (std::string)"TTG2")
    {
        return SIMUG::adv::timeScheme::TTG2;
    }
    else if (name == (std::string)"TTG3")
    {
        return SIMUG::adv::timeScheme::TTG3;
    }
    else if (name == (std::string)"TTG4")
    {
        return SIMUG::adv::timeScheme::TTG4;
    }
    else
    {
        SIMUG_ERR("possible variants for advection time scheme on A-grid: TG2, TTG2, TTG3, TTG4!");
    }
}

SIMUG::adv::spaceScheme GetSpaceScheme(const std::string& name)
{
    if (name == (std::string)"CFE")
    {
        return SIMUG::adv::spaceScheme::CFE;
    }
    else
    {
        SIMUG_ERR("possible variants for advection space scheme on A-grid: CFE!");
    }
}
...
SIMUG::adv::advFilter GetFilter(const std::string& name)
{
    ...
    else if (name == (std::string)"Zalesak")
    {
        return SIMUG::adv::advFilter::Zalesak;
    }
    else
    {
        SIMUG_ERR("possible variants for advection filter on A-grid: none, Zalesak!");
    }
}
