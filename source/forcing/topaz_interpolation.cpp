                              ...
                              INMOST::Tag netcdf_coords_tag)
{
    ...
    {
        tmp_min_x = mesh->GetMesh()->BeginFace()->RealArray(netcdf_coords_tag)[0];
        tmp_max_x = mesh->GetMesh()->BeginFace()->RealArray(netcdf_coords_tag)[0];
        tmp_min_y = mesh->GetMesh()->BeginFace()->RealArray(netcdf_coords_tag)[1];
        tmp_max_y = mesh->GetMesh()->BeginFace()->RealArray(netcdf_coords_tag)[1];

        for(auto edgeit = mesh->GetMesh()->BeginFace(); edgeit != mesh->GetMesh()->EndFace(); ++edgeit) 
	    {
            if(edgeit->GetStatus() != Element::Ghost)
            {
                double current_x = edgeit->RealArray(netcdf_coords_tag)[0];
                double current_y = edgeit->RealArray(netcdf_coords_tag)[1];
    ...
    if ((retval = nc_inq_varid(fileid, file_info.lonname.c_str(), &lon_id)))
    {
        NC_ERR(retval);
    }

    if ((retval = nc_inq_varid(fileid, file_info.latname.c_str(), &lat_id)))
    {
        NC_ERR(retval);
    }
    ...
    else if (grid_type == mesh::gridType::Cgrid)
    {
        for(auto edgeit = mesh->GetMesh()->BeginFace(); edgeit != mesh->GetMesh()->EndFace(); ++edgeit) 
        {
            ...
            if(edgeit->GetStatus() != Element::Ghost)
            {
                double x = edgeit->RealArray(netcdf_coords_tag)[0];
                double y = edgeit->RealArray(netcdf_coords_tag)[1];
